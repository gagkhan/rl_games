...
class CoordConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
        super().__init__(in_channels + 2, out_channels, kernel_size, stride, padding, dilation, groups, bias)
    ...
    def get_coord(x):
        ...
        if key not in CoordConv2d.pool:
            ...
            coord = (
                torch.nn.functional.affine_grid(theta, torch.Size([1, 1, x.size(2), x.size(3)]))
                .permute([0, 3, 1, 2])
    ...
    def forward(self, x):
        return torch.nn.functional.conv2d(
            torch.cat([x, self.get_coord(x).type_as(x)], 1),
            self.weight,
            self.bias,
            self.stride,
            self.padding,
...
class LayerNorm2d(nn.Module):
    ...
    def _check_input_dim(self, input):
        if input.size(1) != self.gamma.nelement():
            raise ValueError("got {}-feature tensor, expected {}".format(input.size(1), self.features))
...
class CategoricalMasked(torch.distributions.Categorical):
    ...
    def rsample(self):
        u = torch.distributions.Uniform(
            low=torch.zeros_like(self.logits, device=self.logits.device),
            high=torch.ones_like(self.logits, device=self.logits.device),
        ).sample()
